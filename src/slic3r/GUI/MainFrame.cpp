...
void MainFrame::load_config_file()
{
 ...
 //       return;
    wxFileDialog dlg(this, _L("Select profile to load:"),
        ...
        "config.json", _L("Config files (*.json;*.zip;*.orca_printer;*.orca_bundle;*.orca_filament;*.qdscfg;*.qdsflmt)|*.json;*.zip;*.orca_printer;*.orca_bundle;*.orca_filament;*.qdscfg;*.qdsflmt"), wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
     wxArrayString files;
    if (dlg.ShowModal() != wxID_OK)
        return;
    dlg.GetPaths(files);
